...
def percent_change_scaling_per_run(img, mask_img, debug=False):
    ...
    if masked_data.size > 0:
        ...
        #masked_pc = (masked_data - voxel_means_safe) / voxel_means_safe
...
def run_single_split(train_imgs, test_imgs, train_designs, test_designs,
    ...
    contrast="face1-(bodylimb1+ES_SC1+ES_RW1+ES_FF1+ES_CS1)/5"
    t_img = fmri_glm.compute_contrast(contrast,stat_type="t")
    t_map = get_data(t_img).astype(np.float32)
...
def run_permutations(
    ...
    for p in tqdm(range(n_permutations), desc="Permutations"):
        ...
        t_perm_sum = np.zeros(mask_data.shape, dtype=np.float32)
        ...
        t_perm_mean = t_perm_sum / len(splits)
    ...
    return max_distribution, r2_perm_mean.astype(np.float32), t_perm_mean.astype(np.float32)
...
def run_cv(
    ...
    t_sum = np.zeros(mask_shape, dtype=np.float32)
    ...
    return mean_r2.astype(np.float32), var_r2.astype(np.float32),mean_t.astype(np.float32)
...
def main():
    ...
    if not args.only_permutations:
        ...
        Nifti1Image(mean_t, mask_img.affine).to_filename(f"{args.output_prefix}_real_mean_t.nii.gz")
        Nifti1Image(mean_r2, mask_img.affine).to_filename(f"{args.output_prefix}_real_mean_r2.nii.gz")
        Nifti1Image(var_r2, mask_img.affine).to_filename(f"{args.output_prefix}_real_var_r2.nii.gz")
        del mean_r2, var_r2, mean_t
        gc.collect()
    ...
    if args.n_permutations > 0:
        ...
        Nifti1Image(mean_r2, mask_img.affine).to_filename(f"{args.output_prefix}_perm_mean_r2.nii.gz")
