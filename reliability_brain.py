...
def reliability_analysis(subject,task,methodx,directory=source_directory,
    ...
    ##############################
    file1=directory+"/"+subject+"_ses-1_"+task+"_OC_part-mag_bold_"+methodx+"_gm_union.nii.gz"
    file2=file1.replace("analysis_timeSeries","analysis")
    ...
    epi_mask=np.ma.masked_where(data_episeries_x[:,0]!=0,data_episeries_x[:,0])
    ...
    if savecorr == True:
        ...
        file_corr2=directory+"/"+subject+"_ses-1_"+task+"_OC_functional_connectivity_"+methodx+"_half2.csv"
    ...
    if hist==True:
        ...
        fig.savefig(directory+"/"+subject+"_ses-1_"+task+"_OC_reliability_coefficients_"+methodx+"_histogram.png")
        plt.close(fig)
    print(f""" Reliability score computed voxel wise between two halves. result has shape: {reliability_vector.shape}""")
    if save == True:
        file_reliability=directory+"/"+subject+"_ses-1_"+task+"_OC_functional_connectivity_"+methodx+"_reliability.csv"
    ...
    if plot == True:
        ...
        brain_reliability=plot_epi(plot_results,colorbar=True,draw_cross=False,cmap="inferno",vmin=0,vmax=0.5)
        brain_reliability.savefig(directory+"/"+subject+"_ses-1_"+task+"_OC_functional_connectivity_"+methodx+"_reliability.png")
