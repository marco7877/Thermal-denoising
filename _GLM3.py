...
# ============================================================
# Utilities (YOUR ORIGINAL FUNCTIONS - UNCHANGED)
# ============================================================
...
# ============================================================
# ORIGINAL run_cv FUNCTION (for real data)
# ============================================================
...
def run_cv(
    ...
    """
    YOUR ORIGINAL run_cv function - unchanged
    """
...
def run_single_permutation_split(train_idx, test_idx, 
                                all_runs_imgs, designs, run_timepoints, 
                                mask_data, n_task_regressors, tr, hrf_model, seed):
    """
    Run a single permutation split and return max R²
    """
    # Get training/test images (already preprocessed!)
    train_imgs = [all_runs_imgs[i] for i in train_idx]
    test_imgs = [all_runs_imgs[i] for i in test_idx]
    
    # Permute design matrices
    rng = np.random.default_rng(seed)
    ...
    # Compute max R²
    r2_values = np.zeros(test_data_masked.shape[0])
...
def run_permutations_optimized(
    nii_files,
    events_files,
    mask_img,
    n_task_regressors,
    ...
    """
    Run permutations efficiently by preprocessing once
    """
    # STEP 1: Preprocess ALL data ONCE (this is the key!)
    print("\n" + "="*60)
    print("PREPROCESSING ALL DATA FOR PERMUTATIONS (ONCE)")
    print("="*60)
    ...
    print(f"  {len(all_runs_imgs)} runs preprocessed")
    print(f"  {len(designs)} design matrices loaded")
    print(f"  Starting {n_permutations} permutations...")
    
    # STEP 2: Generate all tasks
    tasks = []
    base_seed = random_state + 1000  # offset to avoid collision with real data
    
    for p in range(n_permutations):
        for split_id, (train_idx, test_idx) in enumerate(splits):
            seed = base_seed + p * len(splits) + split_id
            tasks.append((p, train_idx, test_idx, seed))
    
    # STEP 3: Run permutations
    if n_jobs > 1:
        results = Parallel(n_jobs=n_jobs)(
            delayed(run_single_permutation_split)(
                train_idx, test_idx,
                all_runs_imgs, designs, run_timepoints,
                mask_data, n_task_regressors, tr, hrf_model, seed
            )
            for p, train_idx, test_idx, seed in tasks
        )
    else:
        results = []
        for p, train_idx, test_idx, seed in tqdm(tasks, desc="Permutations"):
            r2 = run_single_permutation_split(
                train_idx, test_idx,
                all_runs_imgs, designs, run_timepoints,
                mask_data, n_task_regressors, tr, hrf_model, seed
            )
    ...
    # STEP 4: Aggregate results (take max across splits for each permutation)
    max_distribution = np.zeros(n_permutations)
    for (p, _, _, _), r2 in zip(tasks, results):
        if r2 > max_distribution[p]:
            max_distribution[p] = r2
...
def main():
    parser = argparse.ArgumentParser(description="Cross-validated GLM with optimized permutations")
    ...
    print(f"\n{'='*60}")
    print("CROSS-VALIDATED GLM WITH OPTIMIZED PERMUTATIONS")
    print(f"{'='*60}")
    ...
    # Run real data CV (using original run_cv)
    ...
    # Run permutations (optimized!)
    if args.n_permutations > 0:
        print(f"\n{'='*60}")
        print(f"RUNNING {args.n_permutations} PERMUTATIONS (OPTIMIZED)")
        print(f"{'='*60}")
        
        max_distribution = run_permutations_optimized(
            args.nii_files,
            args.events_files,
            mask_img,
            args.n_task_regressors,
